    ...
    <>
      <main>
        <div className="container w-10/12 mx-auto mt-10 flex gap-5 items-center">
          <p className="text-xl font-semibold">Прибыль</p>
          <ul className="flex">
          ...
          {location.pathname === `/admin/profit/${year}/${month}` && (
            <p className="font-semibold ml-4">
              {filteredMonth.charAt(0).toUpperCase() + filteredMonth.slice(1)}
            </p>
          )}
          {location.pathname === `/admin/profit/${year}/${month}/${day}` && (
            <p className="font-semibold ml-4">
              {filteredMonth.charAt(0).toUpperCase() + filteredMonth.slice(1)},{" "}
              {day}
            </p>
          ...
          {location.pathname ===
            `/admin/profit/${year}/${month}/${day}/${user}` && (
            <p className="font-semibold ml-4">
              {filteredMonth.charAt(0).toUpperCase() + filteredMonth.slice(1)},{" "}
              {day}, Клиент ID: {user}
            </p>
